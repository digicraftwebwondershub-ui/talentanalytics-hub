...
<div id="jd-modal" class="modal-overlay">
  <div class="modal-content">
    <div class="flex justify-between items-center mb-4">
...
<div id="tab-area" style="display:none;">
  ...
  <nav id="tab-navigation" class="flex -mb-px" aria-label="Tabs">
    ...
    <div onclick="switchTab('my-requests')" id="tab-my-requests" class="tab reports-view-tab" style="display: none;">My Requests</div>
    <div onclick="switchTab('approvals')" id="tab-approvals" class="tab reports-view-tab" style="display: none;">Approvals</div>
...
<div id="masterlist-view" class="tab-content">
    ...
    <div class="p-4 sm:p-6 bg-white rounded-lg shadow-md">
        <table id="masterlist-table" class="display responsive nowrap" style="width:100%">
            </table>
    </div>
  </div>
  ...
  <div id="competency-view" class="tab-content">
    <div class="p-4 sm:p-6 bg-white rounded-lg shadow-md">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-2xl font-bold text-slate-800">Employee Competency</h2>
        ...
        <div id="individual-profile-view">
            <h3 class="text-xl font-bold text-slate-800 mb-4">Employee Competency Profile</h3>
            <div class="max-w-md">
                <label for="competency-employee-select" class="block text-sm font-medium text-gray-700">Choose a Person</label>
                <select id="competency-employee-select" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
    ...
    <div id="individual-profile-content">
      <div id="competency-summary-container" class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-8" style="display: none;">
      <div class="p-4 sm:p-6 bg-white rounded-lg shadow-md flex flex-col items-center">
          <h3 class="text-xl font-bold text-slate-800 mb-2 text-center">Overall Score</h3>
          <div id="gauge-chart-div" style="width: 250px; height: 250px;"></div>
      </div>
      <div class="p-4 sm:p-6 bg-white rounded-lg shadow-md">
          <h3 class="text-xl font-bold text-slate-800 mb-4 text-center">Top Strengths</h3>
          <ul id="strengths-list" class="space-y-3">
              </ul>
      </div>
      <div class="p-4 sm:p-6 bg-white rounded-lg shadow-md">
          <h3 class="text-xl font-bold text-slate-800 mb-4 text-center">Development Areas</h3>
          <ul id="gaps-list" class="space-y-3">
              </ul>
      </div>
  </div>
  
  <div id="competency-history-container" class="mt-8 p-4 sm:p-6 bg-white rounded-lg shadow-md" style="display: none;">
      <h3 class="text-xl font-bold text-slate-800 mb-4 text-center">Competency Development Over Time</h3>
      <canvas id="historical-competency-chart"></canvas>
  </div>
      ...
      <div id="competency-charts-container" class="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-8" style="display: none;">
          <div class="p-4 sm:p-6 bg-white rounded-lg shadow-md">
              <h3 class="text-xl font-bold text-slate-800 mb-4 text-center">Core Competencies</h3>
              <canvas id="core-competency-chart"></canvas>
          </div>
          <div class="p-4 sm:p-6 bg-white rounded-lg shadow-md">
              <h3 class="text-xl font-bold text-slate-800 mb-4 text-center">Leadership Competencies</h3>
              <canvas id="leadership-competency-chart"></canvas>
          </div>
          <div class="p-4 sm:p-6 bg-white rounded-lg shadow-md lg:col-span-2">
              <h3 class="text-xl font-bold text-slate-800 mb-4 text-center">Technical Competencies</h3>
              <canvas id="technical-competency-chart"></canvas>
          </div>
      </div>
    </div>

    <!-- New Team Competency Heatmap Section -->
    <div id="team-competency-section" class="mt-8 p-4 sm:p-6 bg-white rounded-lg shadow-md" style="display: none;">
        <h2 class="text-2xl font-bold text-slate-800 mb-4">Team Competency Heatmap</h2>
        <div class="max-w-md mb-6">
            <label for="heatmap-department-select" class="block text-sm font-medium text-gray-700">Choose a Department</label>
            <select id="heatmap-department-select" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
        ...
        <div class="modal-content">
            ...
            <div id="deep-dive-content" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    <ul id="top-performers-list" class="space-y-2">
...
<div id="my-requests-view" class="tab-content">
  <div class="p-4 sm:p-6 bg-white rounded-lg shadow-md">
    <h2 class="text-2xl font-bold text-slate-800 mb-4">My Change Requests</h2>
    <table id="my-requests-table" class="display responsive nowrap" style="width:100%">
    </table>
  </div>
</div>
...
<div id="approvals-view" class="tab-content">
  <div class="p-4 sm:p-6 bg-white rounded-lg shadow-md">
    <h2 class="text-2xl font-bold text-slate-800 mb-4">Change Request Approvals</h2>
    <table id="approvals-table" class="display responsive nowrap" style="width:100%">
    </table>
  </div>
</div>
...
function navigateToView(viewType, defaultTabId) {
  ...
  // Control visibility of the main title and tab navigation bar
  const mainTitle = document.getElementById('main-title');
  const tabNav = document.getElementById('tab-navigation');
  const headerControls = document.getElementById('header-controls');

  // Show controls ONLY for org chart view
  headerControls.style.display = (viewType === 'org') ? 'flex' : 'none';

  // This is the key change: competency view should behave like org and predictive views.
    if (viewType === 'org' || viewType === 'predictive' || viewType === 'competency') {
      ...
      // Default for any other views
      mainTitle.style.display = 'block';
      tabNav.style.display = 'flex';
  }

  const allTabs = document.querySelectorAll('#tab-area .tab');
  const viewClassMap = {
    'org': 'org-view-tab',
    'analytics': 'analytics-view-tab',
    'predictive': 'predictive-view-tab',
    'reports': 'reports-view-tab',
    'competency': 'competency-view-tab' // Add this line
  };
...
function drawEmployeeMovementCharts(forceRefresh = false) {
  ...
  function onDataFetched(data) {
    ...
    populateFilters();
    // initializeCompetencyView(); // This will be moved to switchTab
    updateUI();
  }
...
// REVISED - This function now loads all analytics at once.
function initializeCompetencyView() {
    competencyViewInitialized = true;
    const select = document.getElementById('competency-employee-select');
    select.innerHTML = '<option>Loading employees...</option>';

    // Load the Google Gauge chart package
    google.charts.load('current', {packages:['gauge']});

    google.script.run.withSuccessHandler(response => {
        if (response.error) {
            select.innerHTML = `<option disabled selected>Error: ${response.error}</option>`;
            return;
        }
    ...
    }).getCompetencyEmployeeList();

    select.addEventListener('change', (event) => {
        const employeeId = event.target.value;
        if (employeeId) {
            drawCompetencyDashboard(employeeId); // Call the new main function
        } else {
            // Hide all sections if no employee is selected
            document.getElementById('competency-summary-container').style.display = 'none';
            document.getElementById('competency-history-container').style.display = 'none';
        ...
        document.getElementById('individual-profile-content').style.display = 'none';
        }
    });
    ...
    // Populate the department dropdown for the heatmap
    const heatmapDeptSelect = document.getElementById('heatmap-department-select');
    heatmapDeptSelect.innerHTML = '<option value="">-- Select a Department --</option>';
    if (dropdownListData && dropdownListData.departments) {
      dropdownListData.departments.forEach(dept => {
          const option = document.createElement('option');
          option.value = dept;
          option.textContent = dept;
          heatmapDeptSelect.appendChild(option);
      });
    }

    heatmapDeptSelect.addEventListener('change', (event) => {
        const department = event.target.value;
        if (department) {
            drawTeamCompetencyHeatmap(department);
        } else {
            document.getElementById('heatmap-container').innerHTML = '';
        }
    });
    
    // Show the individual profile by default
    individualProfileView.style.display = 'block';
}

function openRequestModal() {
  document.getElementById('request-modal').style.display = 'flex';
  document.getElementById('request-type').addEventListener('change', handleRequestTypeChange);
}
...
function handleRequestTypeChange() {
  ...
  const fileUploadHtml = `
    <div class="md:col-span-2">
      <label class="block font-medium">Supporting Documents</label>
      <div class="mt-1">
        <input type="file" id="support-docs-input" multiple class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
      </div>
      <div id="support-docs-list" class="mt-2 text-sm text-gray-600"></div>
      <p id="file-limit-warning" class="text-red-500 text-xs mt-1" style="display: none;">You can upload a maximum of 5 files.</p>
    </div>
  `;
  ...
  switch (requestType) {
    ...
    case 'Internal Transfer':
      fieldsHtml = `
        <div><label class="block font-medium">Employee</label><div id="employee-request-dropdown" class="searchable-dropdown"></div></div>
        <div><label class="block font-medium">Current Position ID</label><input type="text" id="current-position-id" readonly class="mt-1 p-2 w-full border rounded bg-gray-100"></div>
        <div><label class="block font-medium">New Position ID</label><div id="new-position-dropdown" class="searchable-dropdown"></div></div>
        <div><label class="block font-medium">Effective Date</label><input type="date" id="effective-date-request" class="mt-1 p-2 w-full border rounded"></div>
        <div><label class="block font-medium">Justification</label><textarea id="justification" class="mt-1 p-2 w-full border rounded"></textarea></div>
        ${fileUploadHtml}
      `;
      break;
    case 'Hiring of a new employee (newly created position)':
      fieldsHtml = `
        ...
        <div><label class="block font-medium">New Employee Name</label><input type="text" id="new-employee-name" class="mt-1 p-2 w-full border rounded"></div>
        <div><label class="block font-medium">New Employee ID</label><input type="text" id="new-employee-id" class="mt-1 p-2 w-full border rounded"></div>
        <div><label class="block font-medium">Justification</label><textarea id="justification" class="mt-1 p-2 w-full border rounded" rows="3"></textarea></div>
        ${fileUploadHtml}
      `;
      break;
    case 'Hiring of a new employee (replacement for vacancy)':
      fieldsHtml = `
        <div><label class="block font-medium">Vacant Position ID</label><div id="vacant-position-dropdown" class="searchable-dropdown"></div></div>
        <div><label class="block font-medium">New Employee Name</label><input type="text" id="new-employee-name" class="mt-1 p-2 w-full border rounded"></div>
        <div><label class="block font-medium">New Employee ID</label><input type="text" id="new-employee-id" class="mt-1 p-2 w-full border rounded"></div>
        <div><label class="block font-medium">Effective Date</label><input type="date" id="effective-date-request" class="mt-1 p-2 w-full border rounded"></div>
        <div><label class="block font-medium">Justification</label><textarea id="justification" class="mt-1 p-2 w-full border rounded"></textarea></div>
  ...
  const fileInput = document.getElementById('support-docs-input');
  if (fileInput) {
  ...
  if (requestType.includes('Transfer') || requestType.includes('Promotion')) {
    createSearchableDropdown('employee-request-dropdown', dropdownListData.managers || [], 'name', 'id', (employeeId) => {
      const employee = fullEmployeeData.find(e => e.employeeId === employeeId);
      if (employee) {
        document.getElementById('current-position-id').value = employee.positionId;
      }
    });
    const vacantPositions = fullEmployeeData.filter(p => !p.employeeId).map(p => p.positionId);
    createSearchableDropdown('new-position-dropdown', vacantPositions, null, null, null);
  }
  if (requestType.includes('newly created')) {
    createSearchableDropdown('new-job-title-dropdown', dropdownListData.jobtitle || [], null, null, null);
    createSearchableDropdown('new-level-dropdown', dropdownListData.level || [], null, null, null);
    createSearchableDropdown('reporting-to-request-dropdown', dropdownListData.managers || [], 'name', 'id', null);

    // Dependent dropdowns for new position location
  ...
  if (requestType.includes('replacement')) {
    const vacantPositions = fullEmployeeData.filter(p => !p.employeeId).map(p => p.positionId);
    createSearchableDropdown('vacant-position-dropdown', vacantPositions, null, null, null);
  }
}
...
function submitRequest() {
  ...
  if (files.length === 0) {
    alert('You must add at least one supporting document.');
    return;
  }
  if (files.length > 5) {
    alert('You cannot upload more than 5 documents.');
    return;
  }

  showSavingOverlay('Processing Files...');
  ...
  Promise.all(filePromises).then(fileContents => {
    const requestData = {
      ...
      EmployeeID: document.querySelector('#employee-request-dropdown .searchable-dropdown-input')?.dataset.value,
      EmployeeName: document.querySelector('#employee-request-dropdown .searchable-dropdown-input')?.value,
      CurrentPositionID: document.getElementById('current-position-id')?.value,
      ...
      Division: document.querySelector('#request-division-dropdown .searchable-dropdown-input')?.value,
      Group: document.querySelector('#request-group-dropdown .searchable-dropdown-input')?.value,
      Department: document.querySelector('#request-department-dropdown .searchable-dropdown-input')?.value,
      Section: document.querySelector('#request-section-dropdown .searchable-dropdown-input')?.value,
      ReportingToId: document.querySelector('#reporting-to-request-dropdown .searchable-dropdown-input')?.dataset.value,
      ...
      files: fileContents // Attach file contents
    };
    ...
    google.script.run
      .withSuccessHandler(() => {
        ...
        closeRequestModal();
        if (document.getElementById('my-requests-view').classList.contains('active')) {
          drawMyRequestsTable();
        }
      })
      .withFailureHandler((err) => {
...
function drawTeamCompetencyHeatmap(department) {
    ...
    google.script.run
        .withSuccessHandler(data => {
            ...
            if (data.employeeScores.length === 0) {
                container.innerHTML = '<p class="text-gray-500">No employees with competency data found in this department.</p>';
                return;
            }
            ...
            let headerHtml = '<tr><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r align-bottom">Employee Name</th>';
            data.headers.forEach(header => {
                headerHtml += '<th class="p-2 border-l text-xs font-medium text-gray-500 uppercase" style="white-space: normal; max-width: 200px; vertical-align: bottom; table-layout: auto; width: auto;">' + header + '</th>';
            });
            headerHtml += '</tr>';
            ...
            data.employeeScores.forEach(employee => {
                let rowHtml = `<tr><td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 border-r">${employee.name}</td>`;
                employee.scores.forEach(score => {
                    const color = getHeatmapColor(score);
                    rowHtml += `<td class="px-2 py-4 whitespace-nowrap text-sm text-center font-bold text-white" style="background-color: ${color};">${score}</td>`;
                });
                rowHtml += '</tr>';
        ...
        .getTeamCompetencyHeatmap(department);
}
...
// REPLACED - This is now the main function for the entire tab.
function drawCompetencyDashboard(employeeId) {
    ...
    // Call the new, all-in-one backend function
...
// REVISED - This version specifically adds borderWidth to the Team Average dataset and the onClick handler.
    function renderRadarCharts(radarData) {
        const createChart = (canvasId, data) => {
            ...
            chartInstances[canvasId] = new Chart(ctx, {
                ...
                options: {
                    ...
                    onClick: (event) => {
                        const points = chartInstances[canvasId].getElementsAtEventForMode(event, 'nearest', { intersect: true }, true);
                        if (points.length) {
                            const competencyIndex = points[0].index;
                            const competencyName = chartInstances[canvasId].data.labels[competencyIndex];
        ...
        createChart('core-competency-chart', radarData.core);
        createChart('leadership-competency-chart', radarData.leadership);
        createChart('technical-competency-chart', radarData.technical);
    }

// REVISED - This function now draws two lines: one for Actual Average and one for Gap Average.
    function renderHistoricalChart(history) {
        const container = document.getElementById('competency-history-container');
        const canvasId = 'historical-competency-chart';
        
        // Check if there's at least one dataset with some data.
        ...
        const colors = ['#3b82f6', '#ef4444', '#22c55e', '#f97316']; // Blue, Red, Green, Orange
        
        const datasets = history.datasets.map((dataset, index) => ({
            label: dataset.label,
            data: dataset.data,
            borderColor: colors[index % colors.length],
            backgroundColor: colors[index % colors.length] + '33', // Add transparency for area fill
            tension: 0.1,
            fill: false,
            spanGaps: true // Ensures line continues over null points if any
        }));
        ...
        chartInstances[canvasId] = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                  title: {
                    display: true,
                    text: 'Yearly Competency Averages'
                  }
                },
                scales: {
                    y: {
                        beginAtZero: false, // Allow negative values for gap
                        suggestedMax: 4 // Match your 0-4 competency scale
                    }
                }
            }
        });
...
function drawMyRequestsTable() {
  showSavingOverlay('Loading My Requests...');
  google.script.run
    .withSuccessHandler(data => {
      ...
      const columns = data.myRequests.length > 0 ? Object.keys(data.myRequests[0]).map(h => ({ title: h, data: h })) : [{title: 'No requests found'}];
...
function drawApprovalsTable() {
  showSavingOverlay('Loading Approvals...');
  google.script.run
    .withSuccessHandler(data => {
      ...
      if (data.approvals.length === 0) {
        table.html('<thead><tr><th>Info</th></tr></thead><tbody><tr><td>No pending approvals.</td></tr></tbody>');
        hideSavingOverlay();
        return;
      }

      const columns = Object.keys(data.approvals[0]).map(h => ({ title: h, data: h }));
      columns.push({
        title: 'Actions',
        data: 'RequestID',
        render: function(data) {
          return `<button onclick="handleApprovalAction('${data}', 'Approved')" class="bg-green-500 text-white p-1 rounded text-xs">Approve</button>
                  <button onclick="handleApprovalAction('${data}', 'Rejected')" class="bg-red-500 text-white p-1 rounded text-xs ml-1">Reject</button>`;
        }
      });
    ...
    .withFailureHandler(err => {
      hideSavingOverlay();
      alert('Error loading approvals: ' + err.message);
    })
    .getChangeRequests();
}

function handleApprovalAction(requestId, action) {
  const comments = prompt(`Please provide comments for this action (${action}):`);
  if (comments === null) { // User cancelled the prompt
    return;
  }
  ...
  google.script.run
    .withSuccessHandler(response => {
      hideSavingOverlay();
      alert(response);
      drawApprovalsTable(); // Refresh the table
    })
    .withFailureHandler(err => {
      hideSavingOverlay();
